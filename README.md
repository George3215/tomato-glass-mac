...
奶油白底搭配杏桃、鼠尾草绿和淡紫色块。研究节点按类型分色，日程待办按项目分色；完成项保留文字并使用中性底色。内置开源 Comic Neue 常规 / 粗体字体。保留浅色圆角与磨砂导航，无波点或大面积渐变。

## 🗓️ 日程与目标 · 1.10

浅色圆角面板，按 **课题 → 月目标 → 周目标 → 每日 Todo** 安排研究。提供周日程、月日程、长周期可折叠大纲；支持连续月目标、详细日计划、Todo 勾选和关联专注。
...
| ✨ 自己的风格 | 可调透明度；Comic Neue / 系统字体切换，中文搭配开源霞鹜文楷 Lite。 |
...
自有代码采用 [MIT](LICENSE)。蝴蝶壁纸采用 **CC BY-NC-SA 4.0**，水波移植采用 **PolyForm Noncommercial 1.0.0**；包含这些内容的安装包仅限非商业用途。随包提供 Comic Neue（SIL OFL 1.1），中文使用开源霞鹜文楷 Lite。
