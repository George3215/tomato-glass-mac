...
if [[ "${1:-}" == "--ui" ]]; then
    ...
    xcrun swiftc -module-cache-path .build/module-cache Sources/Countdown.swift Sources/GlassTheme.swift Sources/WindowUI.swift .build/Smoke/AppDelegate.swift .build/Smoke/main.swift -framework AppKit -o "$testapp/Contents/MacOS/PomodoroSmoke"
