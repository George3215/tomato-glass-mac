...
xcrun swiftc -module-cache-path .build/module-cache Sources/Countdown.swift Sources/ActivityLog.swift Tests/main.swift -o .build/countdown-tests
...
if [[ "${1:-}" == "--ui" ]]; then
    ...
    xcrun swiftc -O -module-cache-path .build/module-cache Sources/Countdown.swift Sources/ActivityLog.swift Sources/StatisticsUI.swift Sources/GlassTheme.swift Sources/RippleWater.swift Sources/WindowUI.swift .build/Smoke/AppDelegate.swift .build/Smoke/main.swift -framework AppKit -o "$testapp/Contents/MacOS/PomodoroSmoke"
